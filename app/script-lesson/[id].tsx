...
function buildExercises(items: ScriptItem[]): Exercise[] {
  ...
  batches.forEach((batch, batchIdx) => {
    ...
    // ── STAGE 3: PRACTICE — unguided recognition + recall ──
    batch.forEach((item) => {
      // Recognition: see character → pick sound
      if (allSounds.filter((s) => s !== item.transliteration).length >= 1) {
        exercises.push({
          type: 'recognise',
          item,
    ...
    // Recall: hear/see sound → pick character (for first item)
    ...
    // Recall for second item if batch has 2
    ...
    // ── STAGE 4: MICRO-REVIEW from previous batches ──
    if (batchIdx > 0) {
      ...
      if (allSounds.filter((s) => s !== pick.transliteration).length >= 1) {
        exercises.push({
          ...
          correctAnswer: pick.transliteration,
          label: '⚡ QUICK REVIEW',
        });
      }
    }
    ...
    if (idx % 2 === 0) {
      ...
      if (allChars.filter((c) => c !== item.character).length >= 1) {
        exercises.push({
          ...
          item,
          questionType: 'soundToChar',
          options: shuffleOptions(item.character, allChars),
...
/* ──────────── Guided Recognise Card (with hint) ──────────── */

function GuidedRecogniseCard({
  item,
  options,
  correctAnswer,
  onNext,
  ...
  return (
    <View style={styles.exerciseContainer}>
      ...
      <View style={styles.centerContent}>
        {/* Hint: show the character with its sound */}
        <Card variant="outlined" padding="medium" style={{ marginBottom: Spacing.xl, width: '100%' }}>
          ...
          <View style={{ flexDirection: 'row', justifyContent: 'center', alignItems: 'center', gap: Spacing.md }}>
            ...
            <Text style={[Typography.romanized, { color: colors.romanized }]}>
              {item.transliteration}
            </Text>
          </View>
        </Card>
        ...
        <Text style={[Typography.body, { color: colors.textSecondary, textAlign: 'center', marginTop: Spacing.lg, marginBottom: Spacing.lg }]}>
          Now pick the correct sound
        </Text>
      ...
      {showResult && (
        <View style={styles.bottomButton}>
          ...
          >
            {selected === correctAnswer ? '🎉 Great start!' : `Answer: ${correctAnswer}`}
          </Text>
          <Button title="Continue" onPress={onNext} size="large" fullWidth />
        </View>
      )}
    ...
    <View style={styles.exerciseContainer}>
      ...
      <View style={styles.centerContent}>
        ...
        <View style={{ flexDirection: 'row', justifyContent: 'center', gap: Spacing.md, marginTop: Spacing.md, marginBottom: Spacing.xl }}>
          <TouchableOpacity
            style={[styles.listenHint, { backgroundColor: colors.primary + '15' }]}
            onPress={() => speak(item.character, { audioFile: item.audioFile, rate: 1.0 })}
            activeOpacity={0.7}
          >
        ...
        {missedCount > 0 && (
          <View style={[styles.noteBox, { backgroundColor: colors.warningLight || '#FEF5E7', borderColor: (colors.warning || '#F39C12') + '40', marginTop: Spacing.lg, alignSelf: 'stretch' }]}>
            <Text style={[Typography.caption, { color: colors.text, textAlign: 'center' }]}>
              💪 You retried {missedCount} character{missedCount > 1 ? 's' : ''} — great persistence!
            </Text>
          </View>
        )}
...
export default function ScriptLessonScreen() {
  ...
  const handleNext = () => {
    ...
    // Before recap, inject re-queue for missed items
    if (
  ...
  const handleAnswer = async (correct: boolean) => {
    ...
    if (correct) {
      setScore((s) => s + 1);
      missedIds.current.delete(itemKey(current.item));
    } else {
      missedIds.current.add(itemKey(current.item));
    }
    await updateItemMastery(itemKey(current.item), correct);
  };
